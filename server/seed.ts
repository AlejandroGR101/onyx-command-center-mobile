...
import type {
  ...
  InsertSensorReading, InsertInventoryItem, InsertArAgingItem, InsertShipment,
  InsertLead, InsertVendor, InsertPressLog,
} from "@shared/schema";

// Inserts rows into table only if the table is empty. Idempotent.
async function seedIfEmpty(table: any, rows: any[], label: string) {
  const existing = await db.select().from(table).limit(1);
  if (existing.length > 0) { console.log(`skip ${label}: ya tiene datos`); return; }
  if (rows.length === 0) { console.log(`skip ${label}: sin datos seed`); return; }
...
async function seedUsers() {
  ...
  if (existing.length > 0) { console.log("skip users: admin ya existe"); return; }
...
async function main() {
  // ── Data relocated VERBATIM from MemStorage.seedData() ──
  ...
  // Sensor readings: reproduce the generator loop from MemStorage.seedData() without the `id` field
  const sensorData: InsertSensorReading[] = [];
